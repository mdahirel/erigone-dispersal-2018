...
## Load raw datasets
data_females <- read_csv(here("data","erigone2018_females2.csv"),
...
pairings %>% 
  filter(is.na(mateID)==FALSE) %>% ## filter out the non-mated and the wild-caughts
  group_by(mateID) %>% 
...
```{r}
bf_width_test<-bf(s_CT_width |mi() ~              
             (1|r|ID),                        #
             family=gaussian)

bf_length_test<-bf(s_CT_length |mi() ~   
             (1|r|ID),                        #
             family=gaussian)
...
mod_test=brm(mvbf(bf_width_test+bf_length_test),
        data=data,
        iter=6000,warmup=3000,chains=4,
        prior=prior_test,
        seed=42,
