...
## we make a buffer
buffers1000=st_buffer(points_updated,dist=1000)
...
habitat_avail %>% 
  ...
  scale_y_continuous("")+
  ggtitle("Percentage of area in a 1000m radius around sampling site occupied by *Puccinellia*-dominated meadows")+
  theme_bw()
```
...
newdata_pop<-sites2 %>% 
  ...
  add_fitted_draws(mod_pop)
  
plot2<-ggplot()+
  geom_jitter(data=sites2,aes(landscape,N_adult_females/person_hours_on_patch),
              position=position_jitter(width=0.2),size=2,col="darkgrey")+
...
prior<-c(
  set_prior("normal(0,1)",class="Intercept",resp=c("Nrappelling","sCTwidth","lifetimefecundity")),
  ...
  set_prior("normal(0,1)",class="b",resp=c("Nrappelling","lifetimefecundity","sCTwidth")),
  set_prior("normal(0,1)",class="b",resp=c("timetomaturity","adultlongevity")),
  set_prior("normal(0,1)",class="sd",resp=c("Nrappelling","timetomaturity","lifetimefecundity","adultlongevity","sCTwidth")),
...
newdata_maturity<-data %>% 
  ...
  add_fitted_draws(mod,resp="timetomaturity",re_formula = NA)

plot3_a<-ggplot()+
  geom_boxplot(data=subset(data,is.valid_maturity==1),aes(landscape,time_to_maturity, group=patch))+
  stat_eye(data=newdata_maturity,aes(landscape,.value),
           slab_alpha=0.5,
           .width=c(0.001,0.95),
           point_interval="mean_hdi")+
  scale_x_discrete("")+
...
newdata_size<-data %>% 
  select(landscape) %>% 
  distinct() %>%  
  add_fitted_draws(mod,resp="sCTwidth",re_formula = NA)

plot3_b<-ggplot()+
  geom_boxplot(data=data,aes(landscape,s_CT_width, group=patch))+
  stat_eye(data=newdata_size,aes(landscape,.value),
           slab_alpha=0.5,
           .width=c(0.001,0.95),
           point_interval="mean_hdi")+
  scale_x_discrete("")+
  scale_y_continuous("Adult size (SD units)")

newdata_dispersal<-data %>% 
  select(landscape) %>% 
  distinct() %>% 
  mutate(is.valid_dispersal=1) %>% 
  add_fitted_draws(mod,resp="Nrappelling",re_formula = NA)

plot3_c<-ggplot()+
  geom_boxplot(data=subset(data,is.valid_dispersal==1),aes(landscape,N_rappelling, group=patch))+
  stat_eye(data=newdata_dispersal,aes(landscape,.value),
           slab_alpha=0.5,
           .width=c(0.001,0.95),
           point_interval="mean_hdi")+
  scale_x_discrete("")+
...
newdata_fecundity<-data %>% 
  ...
  add_fitted_draws(mod,resp="lifetimefecundity",re_formula = NA)

plot3_d<-ggplot()+
  geom_boxplot(data=subset(data,is.valid_fecundity==1),aes(landscape,lifetime_fecundity/time_for_repro, group=patch))+
  stat_eye(data=newdata_fecundity,aes(landscape,.value),
           slab_alpha=0.5,
           .width=c(0.001,0.95),
           point_interval="mean_hdi")+
  scale_x_discrete("landscape of origin")+
...
newdata_longevity<-data %>% 
  ...
  add_fitted_draws(mod,resp="adultlongevity",re_formula = NA)

plot3_e<-ggplot()+
  geom_boxplot(data=subset(data,is.valid_longevity==1),aes(landscape,adult_longevity, group=patch))+
  stat_eye(data=newdata_longevity,aes(landscape,.value),
           slab_alpha=0.5,
           .width=c(0.001,0.95),
           point_interval="mean_hdi")+
  scale_x_discrete("")+
